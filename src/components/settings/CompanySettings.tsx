...
const CompanySettings: React.FC = () => {
  ...
  const fetchSettings = async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        const flatSettings: Partial<CompanySettingsData> = {};
        ...
        Object.values(response.data).forEach((group: any) => {
          if (Array.isArray(group)) {
            group.forEach((setting: any) => {
              ...
              // Map keys
              const keyMapping: Record<string, keyof CompanySettingsData> = {
        ...
        // Set logo previews if URLs/paths exist
        if (flatSettings.company_logo) {
          const logoPath = flatSettings.company_logo as string;
          ...
          if (!logoPath.startsWith('data:image/') && !logoPath.startsWith('http')) {
            ...
            setCompanyLogoPreview(`${baseUrl.replace('/api/v1', '')}/storage/${logoPath}`);
          } else {
  ...
  const handleInputChange = (key: keyof CompanySettingsData, value: any) => {
    setSettings(prev => ({ ...prev, [key]: value }));
  };

  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>, type: 'company_logo' | 'app_logo') => {
    ...
    try {
      ...
      const response = await apiService.uploadLogo(file, type);
      toast.dismiss('upload-logo');
      
      if (response.data.success) {
        // Save the file path to settings state
        handleInputChange(type, response.data.data.path);
  ...
  const handleRemoveLogo = async (type: 'company_logo' | 'app_logo') => {
    try {
      toast.loading('Menghapus logo...', { id: 'delete-logo' });
      const response = await apiService.deleteLogo(type);
      toast.dismiss('delete-logo');
      
      if (response.data.success) {
        if (type === 'company_logo') {
          setCompanyLogoPreview(null);
          handleInputChange('company_logo', null);
        } else {
          setAppLogoPreview(null);
  ...
  const handleSave = async () => {
    ...
    try {
      const settingsArray = [
        ...
        { key: 'company_phone', value: settings.company_phone, type: 'string' },
        { key: 'company_email', value: settings.company_email, type: 'string' },
        { key: 'company_website', value: settings.company_website, type: 'string' },
        { key: 'company_npwp', value: settings.company_npwp, type: 'string' },
        { key: 'company_logo', value: settings.company_logo || '', type: 'string' },
      ...
      toast.error(error.message || 'Gagal menyimpan pengaturan perusahaan');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="bg-white shadow rounded-lg">
      <div className="p-6 border-b border-gray-200">
        <h3 className="text-lg font-medium text-gray-900">Pengaturan Perusahaan</h3>
        <p className="mt-1 text-sm text-gray-500">
          Kelola informasi perusahaan, logo, dan pengaturan aplikasi yang digunakan untuk struk, laporan, dan dokumen lainnya.
        </p>
      </div>
      ...
      <div className="p-6 space-y-8">
        ...
        <div>
          <div className="flex items-center mb-4">
            ...
            <h4 className="text-md font-semibold text-gray-900">Informasi Perusahaan</h4>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => handleInputChange('company_npwp', e.target.value)}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500"
        ...
        <div>
          <div className="flex items-center mb-4">
            ...
            <h4 className="text-md font-semibold text-gray-900">Logo Perusahaan</h4>
          </div>
              ...
              <div className="mt-1 flex items-center space-x-4">
                {companyLogoPreview ? (
                  <div className="relative">
                    ...
                    <button
                      type="button"
                      onClick={() => handleRemoveLogo('company_logo')}
        ...
        <div>
          <div className="flex items-center mb-4">
            ...
            <h4 className="text-md font-semibold text-gray-900">Pengaturan Aplikasi</h4>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
